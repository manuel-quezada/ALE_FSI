...
class LevelSetSolver
  ...
  public: 
    ...
    // MOVING MESH //
    /////////////////
    void set_mesh_velocity(PETScWrappers::MPI::Vector locally_relevant_mesh_velocity_vx,
			   PETScWrappers::MPI::Vector locally_relevant_mesh_velocity_vy);
    void set_mesh_velocity(PETScWrappers::MPI::Vector locally_relevant_mesh_velocity_vx,
			   PETScWrappers::MPI::Vector locally_relevant_mesh_velocity_vy,
			   PETScWrappers::MPI::Vector locally_relevant_mesh_velocity_vz);
    ////////////////////////
    // INITIAL CONDITIONS //
    ////////////////////////
  ...
  private:
    ///////////////////////////////////////
    // ASSEMBLE MATRICES FOR MOVING MESH //
    ///////////////////////////////////////
    void assemble_Matrices();
    //////////////////////////
    // ASSEMBLE MASS MATRIX //
    //////////////////////////
    void save_old_mass_matrices();
    void assemble_ML();
    void invert_ML();
    void assemble_MC();    
    ///////////////////////////////////////////
    // ASSEMBLE MATRICES FOR NOT MOVING MESH //
    ///////////////////////////////////////////
    void assemble_C_Matrix();
    void assemble_EntRes_Matrix(); 
    ////////////////////////////
    // OPERATORS TIMES VECTOR //
    ////////////////////////////
    ...
    // MASS MATRIX
    PETScWrappers::MPI::SparseMatrix MC_matrix_tnp1, MC_matrix_tn, update_MC_matrix;
    ...
    // MESH VELOCITY
    PETScWrappers::MPI::Vector locally_relevant_mesh_velocity_vx;
    PETScWrappers::MPI::Vector locally_relevant_mesh_velocity_vy;
    PETScWrappers::MPI::Vector locally_relevant_mesh_velocity_vz;
    PETScWrappers::MPI::Vector locally_relevant_mesh_velocity_vx_old;
    PETScWrappers::MPI::Vector locally_relevant_mesh_velocity_vy_old;
    PETScWrappers::MPI::Vector locally_relevant_mesh_velocity_vz_old;
    // FOR SSP33
    PETScWrappers::MPI::Vector uStage1, uStage2;
    // SOLUTIONS AT DIF TIMES
    ...
    // LUMPED MASS MATRIX
    PETScWrappers::MPI::Vector vector_ML_tnp1, vector_ML_tn, update_vector_ML, ones_vector;
    PETScWrappers::MPI::Vector vector_inverse_ML;
...
/////////////////
template <int dim>
void LevelSetSolver<dim>::set_mesh_velocity(PETScWrappers::MPI::Vector locally_relevant_mesh_velocity_vx,
					    PETScWrappers::MPI::Vector locally_relevant_mesh_velocity_vy)
{
  locally_relevant_mesh_velocity_vx_old = this->locally_relevant_mesh_velocity_vx;
  locally_relevant_mesh_velocity_vy_old = this->locally_relevant_mesh_velocity_vy;
  this->locally_relevant_mesh_velocity_vx=locally_relevant_mesh_velocity_vx;
  this->locally_relevant_mesh_velocity_vy=locally_relevant_mesh_velocity_vy;
}
					    ...
					    PETScWrappers::MPI::Vector locally_relevant_mesh_velocity_vz)
{
  locally_relevant_mesh_velocity_vx_old = this->locally_relevant_mesh_velocity_vx;
  locally_relevant_mesh_velocity_vy_old = this->locally_relevant_mesh_velocity_vy;
  locally_relevant_mesh_velocity_vz_old = this->locally_relevant_mesh_velocity_vz;
  this->locally_relevant_mesh_velocity_vx=locally_relevant_mesh_velocity_vx;
  this->locally_relevant_mesh_velocity_vy=locally_relevant_mesh_velocity_vy;
  this->locally_relevant_mesh_velocity_vz=locally_relevant_mesh_velocity_vz;
}
...
// ------------------------------------------------------------------------------- //
template <int dim>
void LevelSetSolver<dim>::nth_time_step(double time_step)
{ 
  this->time_step=time_step;
  // Matrices for first time step are created at setup
  save_old_mass_matrices();
  assemble_Matrices(); 
  // COMPUTE SOLUTION //
...
void LevelSetSolver<dim>::setup()
{
  ...
  vector_ML_tnp1.reinit(locally_owned_dofs_LS,mpi_communicator);
  vector_ML_tn.reinit(locally_owned_dofs_LS,mpi_communicator);
  update_vector_ML.reinit(locally_owned_dofs_LS,mpi_communicator);
  vector_inverse_ML.reinit(locally_owned_dofs_LS,mpi_communicator);
  ...
  // MESH VELOCITY
  locally_relevant_mesh_velocity_vx.reinit (locally_owned_dofs_U,locally_relevant_dofs_U,mpi_communicator);
  locally_relevant_mesh_velocity_vy.reinit (locally_owned_dofs_U,locally_relevant_dofs_U,mpi_communicator);
  locally_relevant_mesh_velocity_vz.reinit (locally_owned_dofs_U,locally_relevant_dofs_U,mpi_communicator);
  locally_relevant_mesh_velocity_vx_old.reinit (locally_owned_dofs_U,locally_relevant_dofs_U,mpi_communicator);
  locally_relevant_mesh_velocity_vy_old.reinit (locally_owned_dofs_U,locally_relevant_dofs_U,mpi_communicator);
  locally_relevant_mesh_velocity_vz_old.reinit (locally_owned_dofs_U,locally_relevant_dofs_U,mpi_communicator);
  // MESH VELOCITY 
  locally_relevant_mesh_velocity_vx = 0;
  locally_relevant_mesh_velocity_vy = 0;
  locally_relevant_mesh_velocity_vz = 0;
  locally_relevant_mesh_velocity_vx_old = 0;
  locally_relevant_mesh_velocity_vy_old = 0;
  locally_relevant_mesh_velocity_vx_old = 0;
  // LIMITERS (FCT)
  ...
  MC_matrix_tn.reinit (mpi_communicator,
		       dsp,
		       dof_handler_LS.n_locally_owned_dofs_per_processor(),
		       dof_handler_LS.n_locally_owned_dofs_per_processor(),
		       Utilities::MPI::this_mpi_process(mpi_communicator));
  MC_matrix_tnp1.reinit (mpi_communicator,
			 dsp,
			 dof_handler_LS.n_locally_owned_dofs_per_processor(),
			 dof_handler_LS.n_locally_owned_dofs_per_processor(),
			 Utilities::MPI::this_mpi_process(mpi_communicator));
  update_MC_matrix.reinit (mpi_communicator,
			   dsp,
			   dof_handler_LS.n_locally_owned_dofs_per_processor(),
			   dof_handler_LS.n_locally_owned_dofs_per_processor(),
			   Utilities::MPI::this_mpi_process(mpi_communicator));
...
// -------------------------------------------------------------------------------------//
// ------------------------------ MATRIX FOR MOVING MESH ------------------------------ //
// -------------------------------------------------------------------------------------//
template <int dim>
void LevelSetSolver<dim>::assemble_Matrices()
{
  // This function is meant to be used with the moving mesh to assemble everything together
  // assemble:
  //    Cij and Cji matrices
  //    compute MC at tnp1 via update of mass matrix 
  //    assemble EntRes matrix: Rij with SuppSize and entropy normalization
  Cx_matrix=0;
  CTx_matrix=0;
  Cy_matrix=0;
  CTy_matrix=0;
  ...
  std::vector<Tensor<1,dim> > grad_vxqn_mesh (n_q_points); //grad of vxq
  std::vector<Tensor<1,dim> > grad_vyqn_mesh (n_q_points); //grad of vyq
  std::vector<Tensor<1,dim> > grad_vzqn_mesh (n_q_points); //grad of vzq
  ...
  FullMatrix<double>   cell_update_MC (dofs_per_cell_LS, dofs_per_cell_LS);
  ...
  Vector<double>       cell_update_ML (dofs_per_cell_LS);

  std::vector<Tensor<1, dim> > shape_grads_LS_tn(dofs_per_cell_LS);
  std::vector<double> shape_values_LS_tn(dofs_per_cell_LS);
  std::vector<double> shape_values_LS_tnm1(dofs_per_cell_LS);
  ...
  for (; cell_LS!=endc_LS; ++cell_LS, ++cell_U)
	...
	fe_values_LS_tn.get_function_values(un,uqn);
	fe_values_LS_tnm1.get_function_values(unm1,uqnm1);
	fe_values_LS_tn.get_function_gradients(un,guqn);
	fe_values_LS_tnm1.get_function_gradients(unm1,guqnm1);
	
	// physical velocity
	fe_values_U_tn.get_function_values(locally_relevant_solution_vx,vxqn);
	fe_values_U_tn.get_function_values(locally_relevant_solution_vy,vyqn);
	if (dim==3) fe_values_U_tn.get_function_values(locally_relevant_solution_vz,vzqn);
	fe_values_U_tnm1.get_function_values(locally_relevant_solution_vx_old,vxqnm1);
	fe_values_U_tnm1.get_function_values(locally_relevant_solution_vy_old,vyqnm1);
	if (dim==3) fe_values_U_tnm1.get_function_values(locally_relevant_solution_vz_old,vzqnm1);

	// mesh velocity
	fe_values_U_tn.get_function_gradients(locally_relevant_mesh_velocity_vx,grad_vxqn_mesh);
	fe_values_U_tn.get_function_gradients(locally_relevant_mesh_velocity_vy,grad_vyqn_mesh);
	if (dim==3) fe_values_U_tn.get_function_gradients(locally_relevant_mesh_velocity_vz,grad_vzqn_mesh);
	
	for (unsigned int q=0; q<n_q_points; ++q)
	  {
	    Rkn = 1./time_step*ENTROPY(uqn[q])
	      +(vxqn[q]*ENTROPY_GRAD(uqn[q],guqn[q][0])+vyqn[q]*ENTROPY_GRAD(uqn[q],guqn[q][1]))/2.;
	    Rknm1 = -1./time_step*ENTROPY(uqnm1[q])
	      +(vxqnm1[q]*ENTROPY_GRAD(uqnm1[q],guqnm1[q][0])+vyqnm1[q]*ENTROPY_GRAD(uqnm1[q],guqnm1[q][1]))/2.;
	    if (dim==3) 
	      {
		Rkn += 0.5*vzqn[q]*ENTROPY_GRAD(uqn[q],guqn[q][2]);
		Rknm1 += 0.5*vzqnm1[q]*ENTROPY_GRAD(uqnm1[q],guqnm1[q][2]);
	      }
	    div_Un_mesh = grad_vxqn_mesh[q][0]+grad_vyqn_mesh[q][1];
	    if (dim==3)
	      div_Un_mesh += grad_vzqn_mesh[q][2];
	    const double JxW_tnm1 = fe_values_LS_tnm1.JxW(q);
	    const double JxW_tn = fe_values_LS_tn.JxW(q);
	    for (unsigned int i=0; i<dofs_per_cell_LS; ++i)
	      {
		shape_values_LS_tnm1[i] = fe_values_LS_tnm1.shape_value(i,q);
		shape_values_LS_tn[i] = fe_values_LS_tn.shape_value(i,q);
		shape_grads_LS_tn [i] = fe_values_LS_tn.shape_grad (i,q);
	      }
	    for (unsigned int i=0; i<dofs_per_cell_LS; ++i)
	      {
		cell_update_ML (i) += time_step*div_Un_mesh*shape_values_LS_tn[i]*JxW_tn;
		for (unsigned int j=0; j < dofs_per_cell_LS; j++)
		  {
		    cell_Cij_x(i,j) += (shape_grads_LS_tn[j][0])*shape_values_LS_tn[i]*JxW_tn;
		    cell_Cij_y(i,j) += (shape_grads_LS_tn[j][1])*shape_values_LS_tn[i]*JxW_tn;
		    cell_Cji_x(i,j) += (shape_grads_LS_tn[i][0])*shape_values_LS_tn[j]*JxW_tn;
		    cell_Cji_y(i,j) += (shape_grads_LS_tn[i][1])*shape_values_LS_tn[j]*JxW_tn;
		    if (dim==3) 
		      {
			cell_Cij_z(i,j) += (shape_grads_LS_tn[j][2])*shape_values_LS_tn[i]*JxW_tn;
			cell_Cji_z(i,j) += (shape_grads_LS_tn[i][2])*shape_values_LS_tn[j]*JxW_tn;
		      }
		    cell_update_MC (i,j) += time_step*div_Un_mesh*shape_values_LS_tn[i]*shape_values_LS_tn[j]*JxW_tn;
		    cell_EntRes (i,j) += 
		      Rknm1*shape_values_LS_tnm1[i]*shape_values_LS_tnm1[j]*JxW_tnm1
		      +Rkn*shape_values_LS_tn[i]*shape_values_LS_tn[j]*JxW_tn;
		    cell_volume(i,j) += JxW_tn;
		  }
	      }
	    cell_entropy_mass += ENTROPY(uqn[q])*JxW_tn;
	...
	volume += cell_volume(0,0);
	min_entropy = std::min(min_entropy,cell_min_entropy);
	max_entropy = std::max(max_entropy,cell_max_entropy);
	// DISTRIBUTE
	constraints.distribute_local_to_global (cell_update_MC,local_dof_indices_LS,update_MC_matrix);
	constraints.distribute_local_to_global (cell_update_ML,local_dof_indices_LS,update_vector_ML);
  ...
  // Mass matrices
  update_MC_matrix.compress(VectorOperation::add); //This is still just the update
  update_vector_ML.compress(VectorOperation::add);
  // Entropy residual matrices
  EntRes_matrix.compress(VectorOperation::add);
  SuppSize_matrix.compress(VectorOperation::add);
  // update mass matrix
  MC_matrix_tnp1.add(update_MC_matrix,1.0); // old MC_matrix + update_MC
  preconditioner_MC.reset(new PETScWrappers::PreconditionBoomerAMG(MC_matrix_tnp1,PETScWrappers::PreconditionBoomerAMG::AdditionalData(true)));
  vector_ML_tnp1.add(1.0,update_vector_ML);
  invert_ML();
  //ENTROPY NORM FACTOR 
...
// ----------------------------------------------------------------------------//
template<int dim>
void LevelSetSolver<dim>::save_old_mass_matrices()
{
  vector_ML_tn = vector_ML_tnp1;
  MC_matrix_tn.copy_from(MC_matrix_tnp1);
}
...
void LevelSetSolver<dim>::assemble_ML()
{
  vector_ML_tnp1=0;  
  const QGauss<dim>  quadrature_formula(degree_MAX+1);
  FEValues<dim> fe_values_LS (mapping_tn, fe_LS, quadrature_formula,
			      update_values    |  update_gradients | 
  ...
  for (; cell_LS!=endc_LS; ++cell_LS)
	...
	constraints.distribute_local_to_global (cell_ML,local_dof_indices,vector_ML_tnp1);
      }
  // compress
  vector_ML_tnp1.compress(VectorOperation::add);
}
...
void LevelSetSolver<dim>::invert_ML()
{
  ...
  for (;idofs_iter!=locally_owned_dofs_LS.end(); idofs_iter++)
    {
      int gi = *idofs_iter;
      vector_inverse_ML(gi) = 1./vector_ML_tnp1(gi);
    }
  vector_inverse_ML.compress(VectorOperation::insert);
}
...
void LevelSetSolver<dim>::assemble_MC()
{
  ...
  for (; cell_LS!=endc_LS; ++cell_LS)
	...
	// distribute
	cell_LS->get_dof_indices (local_dof_indices);
	constraints.distribute_local_to_global (cell_MC,local_dof_indices,MC_matrix_tnp1);
      }
  // compress
  MC_matrix_tnp1.compress(VectorOperation::add);
  preconditioner_MC.reset(new PETScWrappers::PreconditionBoomerAMG(MC_matrix_tnp1,PETScWrappers::PreconditionBoomerAMG::AdditionalData(true)));
}

// ------------------------------------------------------------------------------------------ //
// ------------------------------ MATRICES FOR NOT MOVING MESH ------------------------------ //
// ------------------------------------------------------------------------------------------ //
template <int dim>
...
void LevelSetSolver<dim>::assemble_K_times_vector(PETScWrappers::MPI::Vector &solution)
{
  ...
  FEValues<dim> fe_values_LS (mapping_tn,fe_LS, quadrature_formula,
			      ...
			      update_quadrature_points |
			      update_JxW_values);
  FEValues<dim> fe_values_U (mapping_tn,fe_U, quadrature_formula,
			     update_values    |  update_gradients |
			     update_quadrature_points |
			     update_JxW_values);
  const unsigned int   dofs_per_cell = fe_LS.dofs_per_cell;
  ...
  std::vector<unsigned int> indices_U (dofs_per_cell);
  const unsigned int dofs_per_cell_U = fe_U.dofs_per_cell;
  ...
  for (; cell_LS!=endc_LS; ++cell_U, ++cell_LS)
	...
	fe_values_U.reinit (cell_U);
	cell_U->get_dof_indices (indices_U);
...
(PETScWrappers::MPI::Vector &solution)
{
  ...
  for (;idofs_iter!=locally_owned_dofs_LS.end(); idofs_iter++)
      ...
      if (dim==3)
	{
      ...
      MatGetRow(MC_matrix_tnp1,gi,&ncolumns,&gj,&MCi);
      ...
      if (dim==3)
	{
      ...
      MatRestoreRow(MC_matrix_tnp1,gi,&ncolumns,&gj,&MCi);
    }
  //compress
 ...
 PETScWrappers::MPI::Vector &un_solution)
{
  ...
  MPP_uL_solution.scale(vector_inverse_ML);
  MPP_uL_solution.add(1.0,un_solution);
  //////////////////////////////////
  // COMPUTE GALERKIN u2 solution //
  //////////////////////////////////
  MC_matrix_tnp1.vmult(RHS,un_solution);
  RHS.add(-time_step,K_times_solution,-time_step,DH_times_solution);
  solve(constraints,MC_matrix_tnp1,preconditioner_MC,NMPP_uH_solution,RHS);
}
 ...
 PETScWrappers::MPI::Vector &solution)
{
  ...
  MPP_uH_solution.scale(vector_inverse_ML);
				...
				const PETScWrappers::MPI::Vector &rhs)
{
  ...
  constraints.distribute (completely_distributed_solution);
  if (verbose==true) pcout << "   Solved LS in " << solver_control.last_step() << " iterations." << std::endl;
}
